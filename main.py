...
# Initialize the AI client (uses OPENAI_API_KEY from environment variables by default)
client = OpenAI()
...
```
    ...
    try:
        ...
        log_error(f"LLM Inference failed: {e} | raw_response={result_text[:300] if 'result_text' in dir() else 'N/A'}")
...
# Always print next scheduled execution on startup
...
# Validate the scheduling window ONLY if execution is not being forced
if not force_execution and datetime.now() < next_dt:
    sys.exit(0)

# --force bypasses: scheduling window, history deduplication, AND the lookback time filter.
# --force only bypasses the history filter (already-sent issues can be re-sent),
# but the time window still applies to avoid surfacing issues that are years old.
...
while True:
    ...
    # 2. Real source code extraction (local first, SonarCloud API as fallback)
...
if not issue or not source_line:
    if force_execution:
        print("❌ Integration error: No open Code Smells were found in your SonarCloud project (no issues in any window, --force bypasses all filters).")
        sys.exit(1)
...
# 3. Live generative AI pass (real LLM inference)
...
# --- 4. HTML TEMPLATE ASSEMBLY (Dynamic and Real) ---
raw_start = max(1, line_number - SOURCE_CONTEXT_LINES)

# Code smell block always uses source_line (Python-extracted, guaranteed original)
code_smell_block = render_code_block(source_line, raw_start, highlight_lines={line_number}, accent_color="#ef4444")

# Detect which lines the LLM modified in suggested_code vs the original source_line.
# Both share the same starting line (raw_start), so index i maps to line raw_start+i.
...
for _i in range(len(_orig_lines), len(_new_lines)):  # highlight extra lines added by the refactor
...
if not force_execution:
    ...
    if success:
        ...
        # On failure, schedule for the next business day as well — a 1-3h retry risks
        # landing outside work hours and creating an infinite failed-retry loop.
        days_to_add = 3 if now.weekday() == 4 else (2 if now.weekday() == 5 else 1)
        next_slot = (now + timedelta(days=days_to_add)).replace(
            hour=random.randint(9, 17),
            minute=random.randint(0, 59),
    ...
    # Always persist the issue key to history, even in forced mode, to prevent re-sending
    if success:
        history.append(issue_key)
        state["last_sent"] = {
            "issue_key": issue_key,
